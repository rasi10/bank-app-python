...
class Bank:
    ...
    def add_customer(self, input_file, name, pnr):
        ...
        with open(input_file, 'a') as file:
            file.write(
                f'{new_customer.id}:{new_customer.name}:{new_customer.pnr}:{account.account_number}:{account.account_type}:{account.balance}\n')
        return True
            ...
            if customer.pnr == pnr:
                ...
                with open(input_file, 'w') as file:
                    for customer in list_of_customers:
                        ...
                        file.write(
                            f'{customer.id}:{customer.name}:{customer.pnr}:{account_string[:-1]}\n')
                return True
        return False
